...
namespace BL_Layer
{
    public class BusinessLogicLayer:DataAccessLayer
    {
        ...
        public int ID;
        public DataTable Login(int Permission, string Username, string Password)
        {
            base.Connect();
            string Query = "SELECT * FROM Tbl_Login WHERE Permission='" + Permission + "' AND Username='" + Username + "' AND Password='" + Password + "'";
